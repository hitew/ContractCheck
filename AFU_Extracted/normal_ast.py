...
class Statement_Norm(object):
    ...
    def load_statement_tokens(self):
        ...
        ancestor_list = []
        statement_list = []
        with open(self.statement_tokens_path ) as st:
    ...
    def store(self):
        ...
        sol = os.path.basename(self.statement_tokens_path)
        with open(store_path + sol + '_normalized', 'w') as snt:
    ...
    def get_token_normalize(self, token_list, token_type):
        '''
            ...
            Parameters : token_list & token_type
                         ...
                         token_type == 15 --> ,
                         token_type == 34 --> .
                         token_type == 115 --> stringLex
                         token_type == 97 --> decimalNum
                         token_type == 98 --> hexNum
                         token_type == 100 --> hexLex
        ...
        for i in range(len(token_list)):
            ...
            # handle the string constant
                ...
                token_list[i] = "stringLex"
                token_list_normalized.append(token_list[i])
            # handle the DecimalNumber
            elif token_type[i] == ",<97>,":
                token_list[i] = "decimalNum"
                token_list_normalized.append(token_list[i])
